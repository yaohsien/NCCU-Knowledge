...
if( $_SESSION['user']!="admin@admin"){
	...
	if (!mysqli_select_db($link, $dbname) )
		...
		if(isset($_GET["id"])){
			...
			$SQLL="SELECT * FROM `tempquestion` WHERE `tempquestion`.`tempID`='$id'";
			$result2 =mysqli_query($link,$SQLL);
			$tempquestion = mysqli_fetch_array($result2); // 用一個 array 去抓值			
			$formalQues=$tempquestion["tempQues"] ;


			switch ($tempquestion["tempAns"] ) {
				case '1':
				$formalAns = 'A';
				break;
				case '2':
			...
			$formalAns1=$tempquestion["tempAns1"] ;
			$formalAns2=$tempquestion["tempAns2"] ;
			$formalAns3=$tempquestion["tempAns3"] ;
			$formalAns4=$tempquestion["tempAns4"] ;
			...
			$SQLL="INSERT INTO `nccuknowledge`.`formalquestion` (`formalID`, `formalQues`, `formalAns`, `formalCat`, `formalAns1`, `formalAns2`, `formalAns3`, `formalAns4`, `formalAnsch1`, `formalAnsch2`, `formalAnsch3`, `formalAnsch4`) 
				   VALUES (NULL, '$formalQues','$formalAns', '','$formalAns1', '$formalAns2', '$formalAns3', '$formalAns4', '0', '0', '0', '0');";
			$result2 =mysqli_query($link,$SQLL);
